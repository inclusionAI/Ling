...
namespace atb_speed {
void SetSkyworkIntParam(uint32_t &fuzzIndex, nlohmann::json &parameter, nlohmann::json &aclParameter)
{
    ...
    std::vector<std::string> routingMethodEnumTable = {"softMaxTopK", "deviceLimited", "integratedSoftmaxTopK"};
...
void SetSkyworkVectorParam(uint32_t &fuzzIndex, nlohmann::json &parameter, nlohmann::json &aclParameter)
{
...
void SetSkyworkSVectorpackParam(uint32_t &fuzzIndex, nlohmann::json &parameter, nlohmann::json &aclParameter)
{
    int numHiddenLayers = *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 1, 1, 28);
    parameter["numHiddenLayers"] = numHiddenLayers;
...
void SetSkyworkSVectorQuantTypeParam(uint32_t &fuzzIndex, nlohmann::json &parameter, nlohmann::json &aclParameter)
{
    int numHiddenLayers = *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 1, 1, 28);
    parameter["numHiddenLayers"] = numHiddenLayers;
...
void SetSkyworkSVectorTransposeTypeParam(uint32_t &fuzzIndex, nlohmann::json &parameter, nlohmann::json &aclParameter)
{
    int numHiddenLayers = *(int *) DT_SetGetNumberRange(&g_Element[fuzzIndex++], 1, 1, 28);
    parameter["numHiddenLayers"] = numHiddenLayers;
...
TEST(SkyworkModelDTFuzz, SkyworkModel)
{
    std::srand(time(NULL));
    std::string fuzzName = "SkyworkModelDTFuzzSkyworkModel";
    ...
    DT_FUZZ_START(0, 10000, const_cast<char*>(fuzzName.c_str()), 0) {
        ...
        parameter["enableFusedRouting"] = FuzzUtil::GetRandomBool(fuzzIndex);
        parameter["enableAddNorm"] = FuzzUtil::GetRandomBool(fuzzIndex);
        SetSkyworkIntParam(fuzzIndex, parameter, aclParameter);
        SetSkyworkVectorParam(fuzzIndex, parameter, aclParameter);
        SetSkyworkSVectorpackParam(fuzzIndex, parameter, aclParameter);
        SetSkyworkSVectorQuantTypeParam(fuzzIndex, parameter, aclParameter);
        SetSkyworkSVectorTransposeTypeParam(fuzzIndex, parameter, aclParameter);

        std::string parameter_string = parameter.dump();
        std::string acl_parameter_string = aclParameter.dump();

        try {
            pybind11::object skyworkFuzz = modelModule.attr("BaseFuzz");
            pybind11::object skyworkFuzzIns = skyworkFuzz("skywork_DecoderModel");
            
            pybind11::object ret = skyworkFuzzIns.attr("set_param")(parameter_string);
            if (ret.cast<int>() == 0) {
                skyworkFuzzIns.attr("set_weight")();
                skyworkFuzzIns.attr("set_kv_cache")();
                skyworkFuzzIns.attr("execute")(acl_parameter_string);
            }
