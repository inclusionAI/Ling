...
class DbrxAttentionConfig(BaseConfig):
    ...
    Args:
        ...
        kv_n_heads (`Optional[int]`, defaults to 1): For grouped_query_attention only, allow user to specify number
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs: Any):
        ...
        if "model_type" in config_dict and hasattr(cls, "model_type") and config_dict["model_type"] != cls.model_type:
            logger.warning(
                f"You are using a model of type has errors."
            )

        return cls.from_dict(config_dict, **kwargs)


class DbrxFFNConfig(BaseConfig):
    ...
    Args:
        ...
        moe_jitter_eps (`float`, *optional*, defaults to `None`): If not `None`, the jitter epsilon for the mixture of
         experts layer.
        moe_loss_weight (`float`, defaults to 0.01): The loss weight for the mixture of experts layer.
        moe_normalize_expert_weights (`float`, *optional*, defaults to 1.0): The normalization factor for the expert
         weights.
    """
...
class DbrxConfig(BaseConfig):
    r"""

    This is the configuration class to store the configuration of a [`DbrxModel`]. It is used to instantiate a
    Dbrx model according to the
    specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a different configuration to that of the [databricks/dbrx-instruct]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if tie_word_embeddings:
            msg = f"Tie word embeddings is not supported for DBRX models."
            logger.error(
                msg,
                ErrorCode.ATB_MODELS_PARAM_OUT_OF_RANGE
            )
        ...
        self.quantization_config.kv_quant_type = None
